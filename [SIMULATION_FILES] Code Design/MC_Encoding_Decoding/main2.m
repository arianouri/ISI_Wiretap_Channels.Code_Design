...
load ENC_Rs09_c2_e5 preCalc_p2 inv_T A B permDex H K M N;

% inner trellis code stage
load TRC_42minerr kINP nOUT MAIN WRTP SupCha_CODEBOOK

MAIN = updateSNR(MAIN,-2);
...
window = 64;
cw_outer_bnd = [zeros(1,window+1) cw_outer zeros(1,window)];

aux = repmat([zeros(1,window+1) cw_outer_punctured zeros(1,window)],nOUT,1);
punDex = find(isnan(aux(:))); clear aux;

[~, ~,MAIN.emsSupCha_out]...
    =SupCha_ENCODER(cw_outer_bnd,MAIN.SupCha_Trans_EX,SupCha_CODEBOOK,kINP,nOUT);
% [~, ~,WRTP.emsSupCha_out]...
%     =SupCha_ENCODER(cw_outer_punctured_truncated_bnd,WRTP.SupCha_Trans_EX,SupCha_CODEBOOK,kINP,nOUT);

%% Complex Gaussian Noise
MAIN_ChannelOutVec=MAIN.emsSupCha_out...
    +   normrnd(0,MAIN.sigma,1,length(MAIN.emsSupCha_out))...
    +1i*normrnd(0,MAIN.sigma,1,length(MAIN.emsSupCha_out));

MAIN_ChannelOutVec(punDex)=0;
MAIN.ChannelOutVec =MAIN_ChannelOutVec;
...
MP_MAT(:,[1 2]) = oneDex;               % 2nd column: VN Index
                                        % 3rd column: V2C mp
                                        % 4rd column: C2V mp
o2i = zeros(1,N+2*window+1);           % going tb executed by inner
...
% VN operations
    ...
    disp(['subIter:', num2str(subDex), ' - BER:',num2str(  sum(sign(tx_est)~=1-2*cw_outer)/N),...
                                       ' - MER:',num2str(  sum(sign(tx_est(isnan(cw_outer_punctured)))~=1-2*cw_outer(isnan(cw_outer_punctured)))/K_s  ) ]);
% CN operation
